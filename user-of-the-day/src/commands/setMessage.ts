...
export const sendSearchMessages = async (
  bot: TelegramBot,
  chatId: TelegramBot.ChatId,
  messagesString: string
) => {
  for await (const value of createMessageGenerator(
    messageStringToArray(messagesString)
  )) {
...
const GROUPS_WITH_RIGHTS = [
  TELEGRAM_GROUP_CREATOR,
  TELEGRAM_GROUP_ADMINISTRATOR,
] as const;
type AcceptedPermission = (typeof GROUPS_WITH_RIGHTS)[number];

const hasPermissions = (
  s: TelegramBot.ChatMemberStatus
): s is AcceptedPermission =>
  GROUPS_WITH_RIGHTS.includes(s as AcceptedPermission);
  ...
  async (msg: CleanedMessage): Promise<void> => {
    ...
    if (!hasPermissions(user.status)) {
      await bot.sendMessage(
        msg.chat.id,
        'Команда доступна только администраторам канала 🤓'
      );
      settingNewMessage = false;
      return;
    }
    try {
      await addCustomChatSearchMessages({
        chatId: msg.chat.id,
        tag,
        messagesString: msg.text as string,
      });
